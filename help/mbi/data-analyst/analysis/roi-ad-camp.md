...
[!DNL Adobe Commerce Intelligence] 让您轻松地 [将广告成本数据与收入数据相结合](../../data-analyst/importing-data/integrations/google-adwords.md) 从您的数据库中。 这有助于您确定哪些营销活动具有最高的投资回报(ROI)。 本主题探讨了评估活动性能的几种不同方法。

## 先决条件

* 导入您的广告成本数据：
   * [连接您的 [!DNL Google AdWords] 到 [!DNL Commerce Intelligence]](../importing-data/integrations/google-adwords.md)：这将同步您的 [!DNL Adwords] 花费 [!DNL Commerce Intelligence]
   * [上传其他广告成本数据](../importing-data/connecting-data/import-offline-ad-data.md)：建议对于没有直接连接器的通道执行此操作 [!DNL Commerce Intelligence]
...
上述分析(1)和(2)将在单独的教程中探讨，主题为 [确定您的主要营销渠道](../analysis/most-value-source-channel.md). 在这里，您可以探索分析(3)以衡量一段时间内的促销活动ROI。 此量度可回答从特定营销活动购买的用户是否产生了足够的生命周期收入以覆盖购买成本。
...
1. 创建 [!UICONTROL Metric] 总和就是你的花费时间
...
1. 使用下拉菜单执行 **总和** 在 `adCost` 中的列 [!DNL Adwords...] 表（更改）的顺序 `date` 列。
...
1. 此报表可显示您的所有时间 [!DNL AdWords] 按活动列出的成本
...
1. 设置 [!UICONTROL Time period] 到 `All-time`、和 [!UICONTROL Interval] 到 `None`
1. 在 `Group by` 选项卡，添加 `campaign` 作为 `grouping field`，然后单击 **`Add All`** 在盒子里
1. 此报表可按营销活动显示您的所有时间注册用户

**3. 创建一个报表，按促销活动对平均用户LTV进行分段：**

1. 在任意功能板中，单击 **[!UICONTROL Add Report > Create report]**
1. 选择 `Average lifetime revenue` 计算平均用户生命周期收入的量度
1. 设置 [!UICONTROL Time period] 到 `All-time`、和 [!UICONTROL Interval] 到 `None`
1. 在 `Group by` 选项卡，添加 `campaign` 或 `utm\_campaign` 作为 [!UICONTROL grouping field]，然后单击 `Add All` 在盒子里
1. 此报表可显示按营销活动划分的平均用户生命周期收入

**最后，通过将以下三个分析整合到一个报表中来计算促销活动ROI：**

1. 在任意功能板中，单击 **[!UICONTROL Add Report > Create new report]**
1. 添加作为输入，使用上面使用的三个量度。 每个用户档案都分配有一个字母(例如，\[`A`\]， \[`B`\]和\[`C`\])
1. [!UICONTROL Cost]：添加指标AdWords成本 — 这是变量\[A\]。 按促销活动列出的退货成本。
1. [!UICONTROL Users]：添加新用户 — 此为变量\[B\]。 这将返回按营销活动划分的用户数。
...
1. 要从中包含的其他潜在量度 [!DNL AdWords] 数据包括  `Impressions` 和 `adClicks` (从 [!DNL AdWords] 数据)，以及总计 `number of orders` 通过特定营销活动制造。
...
开始之前， [提交支持服务单](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/mbi-service-policies.html) 将以下维度联接到行项目表(`sales\_flat\_order\_item, order\_item`)：

* 订单的来源（如果您仅在用户级别跟踪反向链接来源，则加入用户的来源）
* 订单的促销活动（如果您仅在用户级别跟踪反向链接来源，则加入用户的促销活动）
* 订单媒介（如果您仅在用户级别跟踪反向链接来源，则加入用户的媒介）

**1. 现在，首先创建一个图表以返回特定产品的每个促销活动的收入：**

1. 在任意功能板中，单击 **[!UICONTROL Add Report > Create new report]**
1. 选择 `Revenue by items` 计算行项目级别收入的指标
1. 设置 [!UICONTROL Time period] 到 `All-time`、和 [!UICONTROL Interval] 到 `None`
1. 在 `Filter by` 选项卡，添加 `product name 'IN'` 产品 `A`，产品 `B`，产品 `C`， ...”并包含营销活动定向的所有产品名称，并以逗号分隔(例如， `product name 'IN' yellow t-shirt`， `red t-shirt, blue t-shirt`)
1. 在 `Group by` 选项卡，添加 `order's campaign` 或 `order's utm\_campaign` 作为 `grouping` 字段，然后单击 **[!UICONTROL Add All]** 在盒子里
1. 此报表可按营销活动显示特定产品的收入

**2. 要计算ROI，请再次将指标合并到一个报表中：**

1. 在任意功能板中，单击 **[!UICONTROL Add Report > Create new report]**
1. 添加 `Revenue by items` 量度，按照特定产品营销活动报表中的说明进行过滤和分组，然后单击 **[!UICONTROL Hide]** 在量度的标量值下
1. 现在添加 [!DNL AdWords Cost] 量度，遵循过滤器并根据以下说明进行分组： `Ad cost by campaigns` 您在中探索的报告 `User acquisition campaigns` 部分，然后单击 **[!UICONTROL Hide]** 在量度的标量值下
1. 设定好这些量度后，添加公式：
1. [!UICONTROL ROI]：输入公式 `\[A\]/\[B\]`， if `\[A\]` 表示 `Revenue per campaign for specific product(s)` 和 `\[B\]` 表示 `Ad cost by campaigns`. 这会返回（特定产品的收入）/（促销活动成本）的比率
1. [!UICONTROL Return]：输入公式 `\[A\]-\[B\]`. 通过计算（平均用户LTV） — （每次购买的平均成本），返回用户的平均利润
1. （可选） [!UICONTROL Revenue]：取消隐藏 `Revenue by items` 量度，以查看每个营销活动的特定产品的收入
