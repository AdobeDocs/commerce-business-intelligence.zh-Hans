---
title: 访客订单
description: 了解访客订单对您数据的影响，以及在 [!DNL Commerce Intelligence] Data Warehouse中正确考虑访客订单的选项。
...
本主题讨论访客订单对您数据的影响，以及在[!DNL Commerce Intelligence]Data Warehouse中正确考虑访客订单的选项。
...
* **如果允许来宾订单**，则意味着某些订单在`customer\_id`列中没有任何值。 在`orders`表中，只为已注册客户提供了`customer\_id`列的值。 未注册的客户将收到此列的`NULL`（或空白）值。 因此，并非所有订单记录在`customers`表中都有匹配的记录。
...
您可以在上面看到`orders`表中有null `customer\_ids`。 如果您使用`customer\_email`来识别独特客户，则可以看到`erin@test.com`已下三(3)份订单。 因此，您可以基于以下条件在`orders`表上构建`New customers`量度：
