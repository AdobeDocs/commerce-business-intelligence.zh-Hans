---
title: Zendesk技术支持报告
description: 分析Commerce Intelligence中的Zendesk技术支持数据以跟踪票证数量和支持趋势。 可在具有新体系结构客户的Pro计划上获得。
...
* **`[!DNL Zendesk] user's`**&#x200B;表
  ...
  * 
    ...
    * `SQL Calculation` `- case when `A` is not `null` and `A!=`end-user`当`B`不是`null`且`B`类似`%@magento.com`然后`Yes`否则`No`结束时，则`Yes`
