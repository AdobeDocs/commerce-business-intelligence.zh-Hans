---
title: 连接PrestaShop
description: 了解如何将PrestaShop数据连接到 [!DNL Commerce Intelligence]。
exl-id: b6f240c0-9461-4015-a56a-64e8e67392fa
role: Admin, Developer, User
feature: Commerce Tables, Data Warehouse Manager, Data Integration, Data Import/Export
TQID: https://experienceleague.adobe.com/PGMA31KqaIE6bFj8LIkzYyu8KbSt-tnet7fVAn64Ja8
product_v2: id: cc9c1b69-d771-4a04-84d3-df2e3989418fid: eadea719-cf89-469b-a6fd-a236a7138047
feature_v2: id: b0c4e988-b173-423f-88d4-345071a0bce8
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bdid: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2: id: df401a2a-327d-468c-a5e4-b7b7ccd071a0
source-git-commit: 3a6b80d7bcfa5db4d86ab4da81239e3ea804f6ad
workflow-type: tm+mt
source-wordcount: 97
ht-degree: 0%

---

# 连接[!DNL PrestaShop]

![PrestaShop徽标](../../../assets/Prestashop-logo.png)

想要了解您的业务表现或寻找您最有价值的客户获取来源和渠道？ 将您的[!DNL PrestaShop]数据连接到[!DNL Commerce Intelligence]允许您回答这些问题等。

[!DNL PrestaShop]数据库基于[!DNL MySQL]，您可以通过[直接连接](../integrations/mysql-via-a-direct-connection.md)、[[!DNL cPanel]](../integrations/mysql-via-cpanel.md)或[通过`SSH tunnel`](../integrations/mysql-via-ssh-tunnel.md)将其连接到[!DNL Commerce Intelligence]。 Adobe建议使用`SSH`方法，因为它最安全。 有关SSH主机密钥验证，请参阅[SSH主机密钥验证](../integrations/ssh-host-key-verification.md)。

## 相关文档

* [分段和过滤的建议数据维度](../../../best-practices/segment-filter.md)
* [您最宝贵的客户获取来源和渠道是什么？](../../analysis/most-value-source-channel.md)
