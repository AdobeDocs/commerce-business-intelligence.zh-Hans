...
[!DNL MBI] 不仅因为其可视化功能，而且还因为它使您能够将所有数据放在一个Data warehouse中。 甚至位于数据库和集成之外的数据也可以引入 [!DNL MBI] 通过使用Data warehouse管理器中的文件上传工具。
...
1. **`CSV`文件必须使用`UTF-8 encoding`**. 大多数情况下，这并不是问题。 如果您在上传文件时遇到此错误， [请参阅此支持文章](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/resolving-utf-8-errors-for-csv-file-uploads.html?lang=en).
...
将数据上传到之前 [!DNL MBI]，检查它是否根据此部分中的指南进行格式设置。
...
现在，您的电子表格已正确格式化， [!DNL MBI]-friendly，将其添加到您的Data warehouse。
...
是否获得了一些要添加到已上传文件的新数据？ 没有问题 — 您可以轻松地更新并附加数据 [!DNL MBI].
...
1. 单击 **[!UICONTROL Open]** 以开始上传。
   ...
   上传完成后， [!DNL MBI] 将验证文件中的数据结构。 A *成功！* 保存表后，屏幕顶部将显示消息。
...
本文仅介绍了使用导入数据的基础知识，但您可能希望执行一些更高级的操作。 有关格式化和导入金融、电子商务、广告支出和其他类型数据的指导，请参阅相关文章。

此外，文件上传并不是将数据导入 [!DNL MBI]. 此 [数据导入API](https://developer.adobe.com/commerce/services/reporting/import-api/) 函数允许您将任意数据推送到 [!DNL MBI] data warehouse。
