...
# 使用 `Time` 中的选项 `Visual Report Builder`

功能之一 `Visual Report Builder` 是全局的 `Time Range` 和 `Interval` 设置。 利用这些设置，可分析特定时间段内的报表数据。
...
如果要遵循本主题中讨论的一些示例报告，请打开 [`Visual Report Builder`](../data-user/reports/ess-rpt-build-visual.md) 然后再继续。
...
此示例使用名为的自定义维度 `Day number of the month`. 如果要创建此报表，但您的Data warehouse中还没有此维度， [联系支持人员](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/mbi-service-policies.html?lang=en) 以寻求帮助。
...
1. 在下拉菜单中，设置以下内容：
   ...
   * 复制 `Items sold last 28 days` 量度并打开新量度的 `Time Options`. 将选项设置为以下内容：
      ...
      * 接下来，您必须添加计算值，以便根据销售趋势显示时间段(`last 7 days`， `last 28 days`、和 `last promo` 句点)以将其包含在报表中。 您必须为每个时间段执行此操作一次。
