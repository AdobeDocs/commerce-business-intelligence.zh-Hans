...
![使用可视化Report Builder](../assets/Group_By_Report_Builder.png)

如果分析中包含的量度基于不同的数据表构建，则通过弹出窗口可在每个表中选择匹配的数据维度。 此处的目标是查找共享分段值类型的维度：

![使用可视化Report Builder](../assets/Dimension_Editor.png)

## 步骤6：设置`Metric Filters`、`Perspective`和`Time Interval` {#metric-specific}

对于添加到分析的每个量度，您可以添加过滤器、选择相关数据透视并设置`time interval`选项。 要访问这些功能，请单击位于报表中所包含量度旁边的funnel (`Filter`)、眼睛(`Perspective`)和时钟(`Time`)图标。

![使用可视化Report Builder](../assets/Filters_Perspective_Interval_Report_builder.png)
...
![使用可视化Report Builder](../assets/edit-filter-rules.png)
...
![使用可视化Report Builder](../assets/rolling_avg_window.png)
...
![使用可视化Report Builder](../assets/save-to-dashboard.png)
...
![使用可视化Report Builder](../assets/RB_Chart.png)

### 表

![使用可视化Report Builder](../assets/RB_Table.png)

### 数字(`scalar`)

![使用可视化Report Builder](../assets/RB_Scalar.png)
